...
//integer array ‘arr’ of size ‘n’ as its argument
//‘r’ represents the number of rats present in an area, ‘unit’ is the
//amount of food each rat consumes and each ith element of
//array ‘arr’ represents the amount of food present in ‘i+1’ house number,
//where 0 <= i
//Note:
...
//SOLUTION:
intcounthouse(int r,int unit,int arr[],int n){
if(arr[]==NULL)
  return -1;
int food_require=r*unit;
int house=0;food_storage=0;
for(int i=0;i<n;i++)
  house++;
food_store=food_store+arr[i];
if(food_require<=food_store){
return house;}}
reture 0;
}
